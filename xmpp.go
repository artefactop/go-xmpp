...
func (c *Client) Recv() (stanza interface{}, err error) {
	for {
		...
		switch v := val.(type) {
		case *ClientMessage:
			//fmt.Println("Is Message")
			return v, nil
		case *ClientIQ:
			//fmt.Println("Is ClientIQ")
			return v, nil
		case *clientQuery:
			//fmt.Println("Is clientQuery")
			var r Roster
...
// SendStanza add default values if not setted
func (c *Client) SendStanza(stanza interface{}) (n int, err error) {
...
func (c *Client) send(stanza interface{}) (n int, err error) {
	m, err := xml.Marshal(stanza)
	if err != nil {
		return 0, err
	}
	return fmt.Fprint(c.conn, string(m))
}

// SendRaw sends the original text without being wrapped in an XMPP message stanza.
func (c *Client) SendRaw(org string) (n int, err error) {
...
func next(p *xml.Decoder) (xml.Name, interface{}, error) {
	...
	case nsClient + " presence":
		nv = &ClientPresence{}
	case nsClient + " iq":
		//TODO check for XEP's
		fmt.Println("iq and..", se)
		xn, xi, err := parseXep(p)
		if err != nil {
			nv = &ClientIQ{}
		} else {
			fmt.Print("xep name", xn)
			switch v := xi.(type) { //TODO use xn instead
			case *Privacy:
				nv = &ClientPrivacyIQ{
					Privacy: *v,
				}
...
func parseXep(p *xml.Decoder) (xml.Name, interface{}, error) {
	// Read start element to find out what type we want.
	se, err := nextStart(p)
	if err != nil {
		return xml.Name{}, nil, err
